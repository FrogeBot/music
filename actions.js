  ...
  try {
    ...
    if (!voiceChannel) // If the user calling the command is not connected to vc
      return message.channel.send(
        makeEmbed(
          "Error",
          `<@${message.author.id}> - ${options.MSG_UNVIBING} You need to be in a voice channel to play music`
        )
      );
    ...
    if (!permissions.has("CONNECT") || !permissions.has("SPEAK")) { // If the bot lacks either connect or speak perms
      return message.channel.send(
        makeEmbed(
          "Error",
          `<@${message.author.id}> - ${options.MSG_UNVIBING} ${options.BOT_NAME} lacks permissions to join and speak in your voice channel`
        )
      );
    }
    ...
    if (
      ...
      return message.channel.send(
        makeEmbed(
          "Error",
          `<@${message.author.id}> - ${options.MSG_UNVIBING} No videos found`
        )
      );
    }
    ...
    if (isPlaylist) { // If is a playlist
      if (!serverQueue) {
        ...
        message.channel.send(
          makeEmbed(
            "Queued Playlist",
            `<@${message.author.id}> - ${options.MSG_VIBING} **[${playlist.title}](https://www.youtube.com/playlist?list=${playlist.id})** has been added to the queue`
          )
        );
    ...
    if (!serverQueue) {
      ...
      if (serverQueue.songs.length == 1) { // If the song being added means that the queue now only has one song (first in queue and nothing playing)
        ...
        return message.channel.send(
          makeEmbed(
            "Queued Track",
            `<@${message.author.id}> - ${options.MSG_VIBING} **[${song.title}](${song.url})** has been added to the queue`,
    ...
    return message.channel.send(
      makeEmbed(
        "Error",
        `<@${message.author.id}> - ${options.MSG_UNVIBING} Something went wrong`
      )
    );
  }
}
...
function skip(message, serverQueue) {
  if (!message.member.voice.channel) { // If the user calling the command is not connected to vc
    return message.channel.send(
      makeEmbed(
        "Error",
        `<@${message.author.id}> - ${options.MSG_UNVIBING} You have to be in a voice channel to skip`,
      )
    );
  }
  if (!serverQueue || serverQueue.songs.length == 0) { // If no music is playing
    return message.channel.send(
      makeEmbed(
        "Error",
        `<@${message.author.id}> - ${options.MSG_UNVIBING} No music is playing this server`,
      )
    );
  }
  if (message.member.voice.channel.id != serverQueue.voiceChannel.id) { // If the voice channels don't match
    return message.channel.send(
      makeEmbed(
        "Error",
        `<@${message.author.id}> - ${options.MSG_UNVIBING} You're not in the same voice channel as the bot`
      )
    );
  }
  let sPercent = Number(options.SKIP_PERCENT); // Get the percent needed to skip
  ...
  if (serverQueue.skips >= toSkip) { // If enough skips votes are cast
    message.channel.send(
      makeEmbed(
        ...
        `<@${message.author.id}> - ${options.MSG_VIBING} Skipped song`
      )
    );
    ...
    message.channel.send(
      makeEmbed(
        ...
        `<@${message.author.id}> - ${options.MSG_VIBING} ${toSkip-serverQueue.skips} more skip${(toSkip-serverQueue.skips>1)?"s":""} required`
      )
    );
  }
}
...
function stop(message, serverQueue) {
  if (!message.member.voice.channel) { // If the user calling the command is not connected to vc
    return message.channel.send(
      makeEmbed(
        "Error",
        `<@${message.author.id}> - ${options.MSG_UNVIBING} You have to be in a voice channel to stop the music`
      )
    );
  }
  if (!serverQueue || serverQueue.songs.length == 0) { // If no music is playing
  ...
  if (
    options.USE_MUSIC_ROLE != "true" ||
    message.member.roles.cache.find(
      (r) => r.name == options.MUSIC_ROLE_NAME
    ) != undefined ||
    ...
    message.channel.send(
      makeEmbed(
        "Stopping",
        `<@${message.author.id}> - ${options.MSG_VIBING} Stopped music`
      )
    );
    ...
    message.channel.send(
      makeEmbed(
        "Error",
        `<@${message.author.id}> - ${options.MSG_UNVIBING} You don't have the **${options.MUSIC_ROLE_NAME}** role`
      )
    );
  }
}
...
function disconnect(message, guildId) {
  ...
  if (
    ...
    message.channel.send(
      makeEmbed(
        "Disconnected",
        `<@${message.author.id}> - ${options.MSG_VIBING}`
      )
    );
  } else { // If none of the other conditions are met, tell them they lack the DJ role
    message.channel.send(
    ...
    if (
      ...
      if (Number(args) - 1 == 0) { // If attempted to remove currently playing song
        return message.channel.send(
          makeEmbed(
            "Unable to remove",
            `<@${message.author.id}> - ${options.MSG_UNVIBING} You can't remove the currently playing song`
          )
        );
      }
      ...
      if (
        ...
        return message.channel.send(
          makeEmbed(
            "Removed",
            `<@${message.author.id}> - ${options.MSG_VIBING} Removed **[${song.title}](${song.url})** from the queue`
          )
        );
      } else { // If none of the other conditions are met, tell them they lack the DJ role
        return message.channel.send(
      ...
      return message.channel.send(
        makeEmbed(
          "Error",
          `<@${message.author.id}> - ${options.MSG_UNVIBING} There is no song at that index`
        )
      );
    }
  }
  ...
  if (!serverQueue || serverQueue.songs.length == 0) { // If no music is playing
    ...
    if (
      ...
      return message.channel.send(
        makeEmbed(
          "Shuffled",
          `<@${message.author.id}> - ${options.MSG_VIBING} Shuffled the music`
        )
      );
    } else { // If none of the other conditions are met, tell them they lack the DJ role
      message.channel.send(
